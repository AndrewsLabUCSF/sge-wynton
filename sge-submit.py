...
def convert_minutes_to_time(time_input):
    if type(time_input) == str:
        ...
        if 1 < len(time) < 4:
            ...
            assert 0 < len(hour) < 4
            assert len(min) == 2
            allsec = 60 * float(min) + 3600 * float(hour)
            if len(time) == 3:
                assert len(min) == 2
                sec = time[2]
                allsec += float(sec)
        elif len(time) == 1:
            allsec = float(time[0]) * 60
        else:
            raise ValueError('Time format not recognized')
    elif type(time_input) in [int, float]:
        allsec = time_input * 60
    else:
        raise TypeError("Type not recognized for time")
    if allsec <= 60:
        return str(int(allsec))
    else:
        minsec = divmod(allsec, 60)
        min, sec = (int(x) for x in minsec)
        if allsec < 3600:
            return "{:02}:{:02}:{:02}".format(0, min, sec)
        else:
            hourmin = divmod(min, 60)
            hour, min = (int(x) for x in hourmin)
            return "{}:{:02}:{:02}".format(hour, min, sec)
...
def format_job_properties(string):
    ...
    # snakemake option
    wildcards_fmt = '.'.join(f'{k}={v}' for k, v in job_properties['wildcards'].items())
    if job_properties['type'] == 'group':
      return string.format(rulename='snakejob', jobid=job_properties['jobid'])
    return string.format(rulename='.'.join([job_properties['rule'], wildcards_fmt]), jobid=job_properties['jobid'])
    ...
    for skey, sval in source.items():
        ...
        for rkey, rval in resource_mapping.items():
            ...
            if skey in rval:
                ...
                # and "disk_mb".
                if (rkey == 's_vmem') or (rkey == 'h_vmem') or (rkey == 'mem_free'):
                    job_options["resources"].update({rkey : str(sval) + 'M'})
                elif (rkey == 's_fsize') or (rkey == 'h_fsize'):
                    job_options["resources"].update({rkey : str(sval) + 'M'})
                elif (rkey == 'h_rt') or (rkey == "s_rt"):
                    job_options["resources"].update({rkey: convert_minutes_to_time(sval)})
                else:
...
# update pe with threads 
threads_value = job_properties['threads']
if 'pe' in qsub_settings['options']:
    # If 'pe' is present, concatenate 'smp ' with the threads value
    qsub_settings['options']['pe'] += f' {threads_value}'
